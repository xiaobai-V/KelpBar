...
// 子进程末尾需要留一个数据填 '\0'：ssize_t bytes_read = read(fd, read_buf, sizeof(read_buf) - 1); // 给 \0 预留一个位置，防止越界
// 子进程的读端的缓冲区大小不够 -> 多次循环读取
...
int main()
{
    ...
    else
    {
        ...
        ssize_t bytes_write = write(fd, msg, strlen(msg)); // 传入长度是否需要设置为strlen(msg) + 1以把 '\0' 也写入到文件呢
        if (bytes_write == -1)
        {
            perror("write fifo");
            close(fd);
